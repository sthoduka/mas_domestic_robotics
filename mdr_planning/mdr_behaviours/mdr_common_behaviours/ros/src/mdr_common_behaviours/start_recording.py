...
class StartRecording(ScenarioStateBase):
    def __init__(self, save_sm_state=False, **kwargs):
        ScenarioStateBase.__init__(self, 'start_recording',
                                   save_sm_state=save_sm_state,
                                   outcomes=['succeeded', 'failed'])
        self.sm_id = kwargs.get('sm_id', '')
        self.state_name = kwargs.get('state_name', 'start_recording')
    ...
    def execute(self, userdata):
        ...
        if self.msg is not None and self.msg.data == 'e_started':
            return 'succeeded'
        else
            return 'failed'
